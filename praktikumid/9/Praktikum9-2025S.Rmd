...
tickets %>% 
  ...
  mutate(
    fill_color = case_when(
      month %in% c("Mar", "Feb", "May") ~ BLUE2,
      TRUE ~ GRAY9
    )
  ) %>% 
  ggplot(aes(x = tickets, y = month, fill = fill_color)) + 
    geom_bar(stat = "identity") + 
    scale_x_continuous(position = "top", expand = expansion(mult = c(0, 0.05))) +
  coord_capped_cart(top = 'both') +
  scale_fill_identity() + 
...
ggplot(value, aes(x = year, y = value, group = product)) +
  ...
  geom_text(aes(label = value), color = GRAY9, vjust = 0, nudge_y = 15, data = function(.x) { .x %>% filter(product == "Product A")}) +
    ylim(c(0, 450)) +
    ylab("Average price") +
    xlab("Year") + 
    ggtitle("Retail price", subtitle = "Price changes reflect maturation of product") +
...
tickets %>% 
  ...
  coord_capped_cart(top = 'both') +
  scale_fill_identity() + 
  geom_text(aes(label = max(tickets)), color = "white", hjust = 1.25, data = ~ .x %>% filter(month %in% c("Mar", "Feb", "May"))) +
  theme_swd() +
  theme(
    axis.title.y = element_blank(),
    axis.line.y = element_blank(),
...
![](images/clipboard-3412185537.png)
